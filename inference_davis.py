'''
Inference code for TGMOESAM2, on Ref-Youtube-VOS
...
import sys
from models.TGMOESAM2 import build_tgmoesam2
...
def sub_processor(args, model, data, save_path_prefix, img_folder, video_list):
    ...
    # get palette
    # palette_img = os.path.join(args.davis_path, "valid/Annotations/53/00000.png")
    # palette = Image.open(palette_img).getpalette()
    
    palette = []
    for i in range(256):
        palette.extend([i, i, i])  # 灰度调色板，也可以换成彩色
    # from tools.colormap import colormap
    # palette = colormap().astype('uint8').flatten().tolist()  # 扁平化成 list
    ...
    for video in video_list:
        ...
        num_obj = num_expressions // 2

        # 2. for each annotator
        for anno_id in range(2):  # 4 annotators
            ...
            vis_save_path = os.path.join(save_path_prefix, f"anno_{anno_id}_vis", video)  
            ...
            for f in range(out_masks.shape[0]):
                # ====== 用于评估 ======
                mask = out_masks[f]
                img_eval = Image.fromarray(mask, mode='P')  # P 模式，保留类别索引
                img_eval.putpalette(palette)  # 评估调色板
                if utils.is_main_process():
                    img_eval.save(os.path.join(anno_save_path, '{:05d}.png'.format(f)))

                # ====== 可视化黑底白前景 ======
                vis_mask = np.where(mask == 0, 0, 255).astype(np.uint8)  # 背景0，前景255
                img_vis = Image.fromarray(vis_mask, mode='L')  # 灰度图
                if utils.is_main_process():
                    img_vis.save(os.path.join(vis_save_path, '{:05d}.png'.format(f)))

                # 更新进度条（只算主进程）
                if utils.is_main_process():
                    progress.update(1)
                # img_E = Image.fromarray(out_masks[f])  # 保证是 P 模式
                ...
                #     progress.update(1)
                # out_masks: [video_len, h, w], dtype=uint8
                # 背景是0，所有病灶区域置255
                # mask = out_masks[f]
                # mask = np.where(mask == 0, 0, 255)  # 背景保持0，病灶区域置255

                # img_E = Image.fromarray(mask, mode='L')  # 灰度图
                # if utils.is_main_process():
                #     img_E.save(os.path.join(anno_save_path, '{:05d}.png'.format(f)))
                #     progress.update(1)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('TGMOESAM2 evaluation script', parents=[opts.get_args_parser()])
