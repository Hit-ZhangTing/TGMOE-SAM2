...
from models.TGMOE_adapter import TGMOE_adapter
...
class TGMOESAM2(nn.Module):
    def __init__(self,
        ...
        self.tgmoe_adapters = nn.ModuleList()
        for i in range(len(fusion_stages)):
            self.tgmoe_adapters.append(TGMOE_adapter(
    ...
    def compute_decoder_out_no_mem(self, backbone_out: BackboneOutput, idx: int):
        ...
        # B = 1
        # C = pix_feat_no_mem.shape[0]
        # H = pix_feat_no_mem.shape[1]
        # W = pix_feat_no_mem.shape[2]

        # pix_feat_no_mem = pix_feat_no_mem.permute(1, 2, 0).reshape(B, C, H, W)
        
        x = current_vision_feats[-1]
        B = x.size(1)
        C = self.sam.hidden_dim
        H, W = backbone_out.feat_sizes[-1]
        pix_feat_no_mem = (x + self.sam.no_mem_embed).permute(1, 2, 0).view(B, C, H, W)
    ...
    def _early_fusion_stage(self, T, samples, txt, attention_mask):
        ...
        for i, (i_v, i_t) in enumerate(zip(fusion_vis[:-1], fusion_txt[:-1])):
            ...
            if i in self.fusion_stages:
                v = vis.clone()
                t = txt.clone()
                v, t = self.tgmoe_adapters[self.fusion_stages.index(i)](v.permute(0, 3, 1, 2), T, t)
