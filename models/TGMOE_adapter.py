...
import argparse
from models.MoE import SparseMoE_Text 
import torch.nn.functional as F

class TextSparseMoE(nn.Module):
    """
    包装 SparseMoE_cv，使其能处理 [B, T, C] 文本特征。
    """
    def __init__(self, n_embed, num_experts=4, top_k=1):
        super().__init__()
        self.n_embed = n_embed
        self.moe = SparseMoE_cv(
            n_embed=n_embed,
    ...
    def forward(self, t):
        # 输入 t: [B, T, C]
        B, T, C = t.shape
        x = t.permute(0, 2, 1).unsqueeze(2)   # [B, C, 1, T]
        out = self.moe(x)   # 得到 [B, C, 4, 4*T]
        out = F.adaptive_avg_pool2d(out, (1, T))  # [B, C, 1, T]
        out = out.squeeze(2).permute(0, 2, 1)
        return out


class TGMOE_adapter(nn.Module):
    ...
    def __init__(self,
        ...
        self.moe_router = SparseMoE_Text(
            dim=adapter_channels,  # adapter feature dim
            num_experts=4,
            top_k=moe_topk
        )
    ...
    def forward(self, vis, clip_length, text):
        ...
        t_moe = self.moe_router(t)
        t_atten = self.ca_T2V(t_moe, einops.rearrange(x, 'hw (b t) c -> hw b t c', t=clip_length).mean(2))  # einops.rearrange(x, '(h w) (b t) c -> (h w) b t c', t=T)
